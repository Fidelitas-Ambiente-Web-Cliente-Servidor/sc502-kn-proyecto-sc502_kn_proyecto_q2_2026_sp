        ...
        <div class="bg-white rounded-4 shadow-sm p-4 mb-4 border border-light-subtle">
          ...
          <nav class="nav flex-column gap-2 mb-0">
            ...
            <a href="index.php?action=rescatista&tab=solicitudes"
              ...
              <span
                class="fw-semibold <?= (isset($_GET['tab']) && $_GET['tab'] == 'solicitudes') ? '' : 'text-secondary' ?>"><i
                  class="fa-regular fa-file-lines me-3"></i>Solicitudes</span>
                    ...
                    <div class="d-flex flex-column flex-md-row justify-content-between gap-4">
                      ...
                      <div class="d-flex align-items-start gap-3">
                        ...
                        <div>
                          ...
                          <p class="text-muted small mb-2"><i class="fa-regular fa-user me-1"></i> De: <span
                              class="fw-semibold text-dark"><?= htmlspecialchars($sol['adoptante_nombre'] . ' ' . $sol['adoptante_apellido']) ?></span>
                            (<?= htmlspecialchars($sol['adoptante_correo']) ?>) <br><i
                              class="fa-solid fa-phone me-1 mt-1"></i> <?= htmlspecialchars($sol['adoptante_telefono']) ?></p>
                      ...
                      <div class="text-md-end d-flex flex-column justify-content-between align-items-md-end"
                        ...
                        if ($sol['estado_solicitud'] == 'Rechazada')
                          $estadoBadge = 'bg-danger';
                        ?>
                        <div class="mb-3">
                          <span id="badge-solicitud-<?= $sol['solicitud_id'] ?>"
                            class="badge <?= $estadoBadge ?> rounded-pill mb-1 px-3 py-2"><?= htmlspecialchars($sol['estado_solicitud']) ?></span><br>
                          <small class="text-muted"><?= date('d M Y', strtotime($sol['fecha_envio'])) ?></small>
                        </div>

                        <?php if ($sol['estado_solicitud'] == 'Pendiente'): ?>
                          <div id="cont-botones-<?= $sol['solicitud_id'] ?>" class="d-flex gap-2 w-100 justify-content-md-end mt-2 mt-md-0">
                            <!-- forma anterior sincrona sin ajax enviaba post normal y recargaba pagina -->
                            <!--
                            <form action="index.php?action=solicitud_estado" method="POST" class="w-100 w-md-auto">
                              ...
                              <button type="submit"
                                class="btn btn-sm btn-outline-success rounded-pill fw-semibold px-3 w-100">Aprobar</button>
                            </form>
                            <form action="index.php?action=solicitud_estado" method="POST" class="w-100 w-md-auto">
                              <input type="hidden" name="solicitud_id" value="<?= $sol['solicitud_id'] ?>">
                              <input type="hidden" name="estado" value="Rechazada">
                              <button type="submit"
                                class="btn btn-sm btn-outline-danger rounded-pill fw-semibold px-3 w-100">Rechazar</button>
                            </form>
                            -->

                            <!-- metodo ajax asincrono cambia estado en tiempo real sin recargar -->
                            <button type="button"
                              class="btn btn-sm btn-outline-success rounded-pill fw-semibold px-3 w-100 w-md-auto btn-ajax-estado"
                              data-id="<?= $sol['solicitud_id'] ?>" data-estado="Aprobada">Aprobar</button>
                            <button type="button"
                              class="btn btn-sm btn-outline-danger rounded-pill fw-semibold px-3 w-100 w-md-auto btn-ajax-estado"
                              data-id="<?= $sol['solicitud_id'] ?>" data-estado="Rechazada">Rechazar</button>
                          </div>
                        <?php endif; ?>
                      </div>
